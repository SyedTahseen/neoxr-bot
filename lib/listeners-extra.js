   ...
   try {
      ...
      client.register('message', ctx => {
         ...
         try {
            ...
            const rawMsg = {
               key: {
                  remoteJid:      rawLid,     // raw chat identity — matches core bot's own chat JID exactly
                  remoteJidAlt:   m.key.remoteJidAlt,   // Baileys-supplied PN counterpart for this chat, when remoteJid is @lid
                  fromMe:         m.key.fromMe ?? false,
                  id:             m.key.id,
                  participant:    senderJid,  // core bot's own resolved sender identity — matches its "from" field exactly
                  participantAlt: m.key.participantAlt, // Baileys-supplied PN counterpart for the participant, when it's @lid
               },
               message:          m.message  || {},
               pushName:         m.pushName || '',
