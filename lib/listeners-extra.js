...
export default (system, client) => {
   try {
      ...
      client.register('message', ctx => {
         baileys(client.sock, ctx)
         if (handler) handler(client.sock, { ...ctx, system })

         // ── Telegram Bridge: forward WhatsApp → Telegram ─────────────────────
         if (!global.telegramBridge) return
         try {
            const m = ctx.m
            if (!m || !m.key) return

            const rawLid    = m.chat || m.key.remoteJid
            const senderJid = m.sender || m.key.participant || rawLid
            const isLidDm   = rawLid.endsWith('@lid') && !m.isGroup
            // Only substitute the real JID for INCOMING messages. For outgoing (fromMe)
            // messages, m.sender is our own number, not the chat partner — using it here
            // would misidentify the chat as "yourself" and create a bogus Telegram topic.
            const remoteJid = isLidDm && senderJid && !senderJid.endsWith('@lid') && !m.key.fromMe
               ? senderJid
               : rawLid
            ...
            const rawMsg = {
               key: {
                  ...
                  id:          m.key.id,
                  participant: (m.isGroup || remoteJid === 'status@broadcast') ? senderJid : undefined,
               },
               message:          m.message  || {},
               pushName:         m.pushName || '',
            ...
            const text         = (typeof m.text === 'string' ? m.text : '') || (typeof m.body === 'string' ? m.body : '')
            const participants = ctx.groupMetadata?.participants || []

            global.telegramBridge.syncMessage(rawMsg, text, participants).catch(err => {
               console.error('\x1b[31m[BRIDGE]\x1b[0m Error forwarding message:', err?.message || err)
            })
         } catch (err) {
            console.error('\x1b[31m[BRIDGE]\x1b[0m Error forwarding message:', err?.message || err)
         }
      ...
      client.register('group.add', async ctx => {
         try {
            ...
            if (groupSet && groupSet.welcome) sock.sendMessageModify(ctx.jid, txt, null, {
               ...
               icon: global.db.setting.icon ? Utils.isUrl(global.db.setting.icon) ? global.db.setting.icon : Buffer.from(global.db.setting.icon, 'base64') : null
            })
         } catch (e) {
            Utils.printError(e)
         }
